var Creatives =
{
    get: function(params)
    {
        ...
        // tblSrc += "<td><div class='fix-height'><a href='" + response.creative[i].field_products_link_author + "' target='_blank'>" + (response.creative[i].field_products_link_author || "") + "</div></td>"
        ...
        // tblSrc += "<td><div class='fix-height'>" + (response.creative[i].field_products_link_cat || "") + "</div></td>"
        ...
        // tblSrc += "<td><div class='fix-height'>" + (response.creative[i].field_products_link_sub_cat || "") + "</div></td>"
        ...
        tblSrc += "<td><div class='fix-height'>" + (response.creative[i].field_created || "") + "</div></td>"
                tblSrc += "<td> \
                        <a href=\"edit/" + response.creative[i].field_products_slug + "\"><span class=\"glyphicon glyphicon-pencil\"></span></a> \
...
console.log(product_name);
        ...
        // // tblSrc += "<td><div class='fix-height'>" + (response.creative[i].field_products_link_sub_cat || "") + "</div></td>"
        // tblSrc += "<td><div class='fix-height'><textarea rows='5' cols='20'>" + (response.creative[i].field_products_desc || "") + "</textarea></div></td>"
        ulSrc += "<a href='product/" + response.creative[i].field_id + " '>";
        ...
        ulSrc += "</figure>";
        ulSrc += "</a>";
        //ulSrc += "<td><div class='fix-height'>" + (response.creative[i].field_created || "") + "</div></td>"
             
                ulSrc += "</li>";
             // ulSrc += "<td> \
             //            <a href=\"portfolio-post/" + response.creative[i].field_id + "\"><span class=\"glyphicon glyphicon-pencil\"></span></a> \
             //            <a href=\"javascript:Creatives.remove(" + response.creative[i].field_id + ");\" style=\"margin-left:10px;\"><span class=\"glyphicon glyphicon-remove\"></span></a> \
             //            </td>";      
            }
            ...
            for (i = Math.max(1, $("#hddnPage").val() - 2);i <= Math.max(5, Math.min(response.pages, parseInt($("#hddnPage").val()) + 2));i++) {
        pgSrc += "<li class=\"" + "animated " + (i == $("#hddnPage").val() ? "active" : "") + "\"><a href=\"javascript:Creatives.show_image({page:" + i + "})\">" + i + "</a></li>";
        }
    ...
    { //console.log(field_products_slug);
...
//console.log(response);
        ...
        $(".field_product_tags").text(response.field_product_tags);
...
response.field_products_requirements,response.field_products_dimensions,response.field_products_dpi,response.field_products_layered];
        ...
        //  if (response.field_products_web_font == 'no') {
        //     $("li .field_products_web_font").parent().addClass('hide');
        // };   
        
        //console.log($('.ul_info li').length);

        //console.log($('.div2 li:nth-child(2)').parent().text());
    ...
    add: function()
    {
        $.getJSON(mainUrl +"front/" + "includes/receiver.php?req=add" + General.packForm("frmCreatives"), function(response) {
            document.location = mainUrl + "creative";
        });
    },
    
    update: function()
    {
        $.getJSON(mainUrl +"admin/" + "includes/receiver.php?req=update" + General.packForm("frmCreatives"), function(response) {
            document.location = mainUrl + "creative";
        });
    },
    ...
    remove: function(field_id)
    {
        if (confirm("Are you sure you want to delete this record")) {
            $.getJSON(mainUrl +"front/" + "includes/receiver.php?req=remove&field_id=" + field_id, function (response) {
